...
export const generateToken = async({payload={},signature=process.env.ACCESS_USER_TOKEN_SIGNATURE , options=Number(process.env.ACCESS_TOKEN_EXPIRES_IN)} = {}) => {
...
let signatures = await getSignatures({signatureLevel:bearer})
        ...
        const decoded = await verifyToken({ token, secret:tokenType === tokenTypeEnum.access ? signatures.accessSignature : signatures.refreshSignature })
        if (!decoded?._id) {
...
await DBService.create({model:TokenModel,data:[{
    jti:req.decoded.jti,
    // logout after 1 year
