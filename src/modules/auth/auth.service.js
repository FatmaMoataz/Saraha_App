...
export const signup = asyncHandler(async (req, res, next) => {
  ...
  const exists = await DBService.findOne({ model: UserModel, filter: { email } })
  if (exists) {
    const error = new Error("Email already exists")
    error.cause = 409
    error.err_message = "Email already exists"
    return next(error)
  }
...
const updatedUser=await DBService.updateOne({
  ...
  return updatedUser.matchedCount ? successResponse({res, status:201, data:{}}) : next(new Error("Failed to confirm user email"))
})
...
export const login = asyncHandler(async (req, res, next) => {
  ...
  if (!user) {
    ...
    error.cause = 401
    error.err_message = "Invalid email or password"
    return next(error)
  }

  if(!user.confirmEmail) {
    return next("please verify your account")
  }
  ...
  const isValid = await compareHash({ plaintext: password, hashValue: user.password })
  if (!isValid) {
    const error = new Error("Invalid email or password")
    error.cause = 401
    error.err_message = "Invalid email or password"
